...
import { getCategoryBySlug, allCategories } from "@/data/butlerCategoryData";
import type { CategoryProduct } from "@/data/butlerCategoryData";
...
export default function ButlerCategoryPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          ...
          <Link to="/butler" className="underline" style={{ color: BU_BLUE }}>Back to Store</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          ...
          fill={i < Math.floor(rating) ? BU_BLUE : "transparent"}
          stroke={i < Math.floor(rating) ? BU_BLUE : "#9ca3af"}
          strokeWidth={1.5}
        />
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* ── TOP BAR ──────────────────────────────────── */}
      <header className="border-b border-border/30" style={{ backgroundColor: BU_DARK }}>
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button className="md:hidden text-white" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
              {mobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
            </button>
            <Link to="/butler" className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs ml-1.5 font-medium px-1.5 py-0.5 rounded" style={{ backgroundColor: BU_BLUE, color: BU_WHITE }}>Butler</span>
              </div>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center flex-1 max-w-xl mx-6">
            <div className="relative w-full">
              ...
              <input
                ...
                style={{ ["--tw-ring-color" as string]: `${BU_BLUE}80` }}
              />
            </div>
          </div>

          <Link to="/butler/cart" className="relative text-white hover:opacity-80 transition-opacity">
            <ShoppingCart className="w-6 h-6" />
            {totalItems > 0 && (
              <span className="absolute -top-2 -right-2 w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center" style={{ backgroundColor: BU_BLUE, color: BU_WHITE }}>
                {totalItems}
              </span>
            )}
          </Link>
      ...
      <div className="container mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <Link to="/butler" className="hover:underline">Store</Link>
          <ChevronRight className="w-3 h-3" />
          {activeSubcategory !== "all" ? (
            <>
              <Link to={`/butler/category/${slug}`} className="hover:underline">{category.name}</Link>
              <ChevronRight className="w-3 h-3" />
      ...
      <div className="container mx-auto px-4 pb-4">
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
          ...
          {category.subcategories.map((sub) => (
            <Link
              key={sub.slug}
              to={`/butler/category/${slug}/${sub.slug}`}
              className={`shrink-0 text-xs font-semibold px-3.5 py-1.5 rounded-full border transition-colors ${
            ...
            <div className="sticky top-4 space-y-5">
              <div className="flex items-center justify-between">
                ...
                {totalActiveFilters > 0 && (
                  <button onClick={clearFilters} className="text-[11px] font-medium hover:underline" style={{ color: BU_BLUE }}>
                    Clear all
                  </button>
                )}
              </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {totalActiveFilters > 0 && (
                    <span className="w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center" style={{ backgroundColor: BU_BLUE, color: BU_WHITE }}>
                      {totalActiveFilters}
                    </span>
                  )}
                </button>
            ...
            {mobileSidebarOpen && (
              <div className="lg:hidden mb-4 p-4 border border-border rounded-lg bg-card space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    {totalActiveFilters > 0 && (
                      <button onClick={clearFilters} className="text-[11px] font-medium" style={{ color: BU_BLUE }}>
                        Clear all
                      </button>
                    )}
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-16">
                ...
                <button onClick={clearFilters} className="text-sm font-medium mt-2 hover:underline" style={{ color: BU_BLUE }}>
                  Clear all filters
                </button>
              </div>
            ) : (
      ...
      {/* ── NIL BANNER ───────────────────────────────── */}
      <div className="border-t border-border" style={{ backgroundColor: BU_DARK }}>
        <div className="container mx-auto px-4 py-6 text-center">
          <p className="text-white/60 text-xs mb-1">Every purchase supports</p>
          <p className="text-lg font-bold text-white">
            600+ Butler Athletes across 20 Sports
          </p>
          <p className="text-xs mt-1" style={{ color: BU_BLUE }}>Every order directly supports Butler NIL</p>
        </div>
      </div>
...
function ProductCard({
  ...
  const productLink = `/butler/category/${categorySlug}/product/${product.id}`;
  if (!gridView) {
    // List view
    return (
      <div className="flex gap-4 p-3 rounded-lg border border-border bg-card hover:shadow-md transition-shadow">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            {product.isNameBrand && (
              <span className="shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: `${BU_BLUE}20`, color: BU_BLUE }}>
                Name Brand
              </span>
            )}
          </div>
          ...
          <div className="mt-2">
            ...
            <p className="text-[10px] font-semibold mt-0.5" style={{ color: BU_BLUE }}>
    ...
    <div className="group rounded-lg border border-border bg-card overflow-hidden hover:shadow-lg transition-all duration-300">
      ...
      <div className="p-3">
        ...
        <div className="mb-2.5">
          <span className="text-base font-bold">${product.price.toFixed(2)}</span>
          <p className="text-[9px] font-semibold mt-0.5" style={{ color: BU_BLUE }}>
            FanPact NIL Contribution: ${product.nilDonation.toFixed(2)}
          </p>
        </div>
        <button
