...
import { getCategoryBySlug, allCategories } from "@/data/indianaCategoryData";
import type { CategoryProduct } from "@/data/indianaCategoryData";
...
export default function IndianaCategoryPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          ...
          <Link to="/indiana" className="underline" style={{ color: IU_CRIMSON }}>Back to Store</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          ...
          fill={i < Math.floor(rating) ? IU_CRIMSON : "transparent"}
          stroke={i < Math.floor(rating) ? IU_CRIMSON : "#9ca3af"}
          strokeWidth={1.5}
        />
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* ── TOP BAR ──────────────────────────────────── */}
      <header className="border-b border-border/30" style={{ backgroundColor: IU_CRIMSON }}>
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button className="md:hidden text-white" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
              {mobileMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
            </button>
            <Link to="/indiana" className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-xs ml-1.5 font-medium px-1.5 py-0.5 rounded bg-white" style={{ color: IU_CRIMSON }}>Indiana</span>
              </div>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center flex-1 max-w-xl mx-6">
            <div className="relative w-full">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 rounded-lg bg-white/10 border border-white/20 text-white placeholder:text-white/50 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-300/50"
              />
            </div>
          </div>
      ...
      <div className="container mx-auto px-4 py-3">
        <nav className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <Link to="/indiana" className="hover:underline">Store</Link>
          <ChevronRight className="w-3 h-3" />
          {activeSubcategory !== "all" ? (
            <>
              <Link to={`/indiana/category/${slug}`} className="hover:underline">{category.name}</Link>
              <ChevronRight className="w-3 h-3" />
      ...
      <div className="container mx-auto px-4 pb-4">
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
          <Link
            ...
            style={activeSubcategory === "all" ? { backgroundColor: IU_CRIMSON } : {}}
          >
            All {category.name}
          </Link>
          {category.subcategories.map((sub) => (
            <Link
              key={sub.slug}
              to={`/indiana/category/${slug}/${sub.slug}`}
              className={`shrink-0 text-xs font-semibold px-3.5 py-1.5 rounded-full border transition-colors ${
                activeSubcategory === sub.slug
                  ? "text-white border-transparent"
                  : "text-foreground border-border hover:border-foreground/40"
              }`}
              style={activeSubcategory === sub.slug ? { backgroundColor: IU_CRIMSON } : {}}
            >
              {sub.label}
            </Link>
          ))}
            ...
            <div className="sticky top-4 space-y-5">
              <div className="flex items-center justify-between">
                ...
                {totalActiveFilters > 0 && (
                  <button onClick={clearFilters} className="text-[11px] font-medium hover:underline" style={{ color: IU_CRIMSON }}>
                    Clear all
                  </button>
                )}
              </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {totalActiveFilters > 0 && (
                    <span className="w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center text-white" style={{ backgroundColor: IU_CRIMSON }}>
                      {totalActiveFilters}
                    </span>
                  )}
                </button>
            ...
            {mobileSidebarOpen && (
              <div className="lg:hidden mb-4 p-4 border border-border rounded-lg bg-card space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    {totalActiveFilters > 0 && (
                      <button onClick={clearFilters} className="text-[11px] font-medium" style={{ color: IU_CRIMSON }}>
                        Clear all
                      </button>
                    )}
                      ...
                      {filter.options.map((opt) => {
                        ...
                        return (
                          <button
                            ...
                            style={active ? { backgroundColor: IU_CRIMSON } : {}}
                          >
                            {opt}
                          </button>
                        );
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-16">
                ...
                <button onClick={clearFilters} className="text-sm font-medium mt-2 hover:underline" style={{ color: IU_CRIMSON }}>
                  Clear all filters
                </button>
              </div>
            ) : (
      ...
      {/* ── NIL BANNER ───────────────────────────────── */}
      <div className="border-t border-border" style={{ backgroundColor: IU_CRIMSON }}>
      ...
      <footer className="bg-gray-900 text-white/60 py-8">
        <div className="container mx-auto px-4 text-center text-xs">
          ...
          <p>Not affiliated with or endorsed by Indiana University.</p>
        </div>
      </footer>
    </div>
  );
      ...
      <div className="flex gap-4 p-3 rounded-lg border border-border bg-card hover:shadow-md transition-shadow">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${IU_CRIMSON}15`, color: IU_CRIMSON }}>
              NIL Donation: ${product.nilDonation.toFixed(2)}
            </span>
          </div>
        </div>
      </div>
    ...
    <div className="group rounded-lg border border-border bg-card overflow-hidden hover:shadow-lg transition-all duration-300">
      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between mb-2.5">
          <span className="text-base font-bold">${product.price.toFixed(2)}</span>
          <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: `${IU_CRIMSON}15`, color: IU_CRIMSON }}>
